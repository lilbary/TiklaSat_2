...
INSERT INTO listings (id, seller_id, category_id, title, description, status, view_count, price, created_at) VALUES
    (gen_random_uuid(),
     ...
     'Yazılımcıdan temiz MacBook. Pil döngüsü 40. Kutusu var.',
     'APPROVED', 150, 65000.00, now()),

    (gen_random_uuid(),
     (SELECT id FROM users WHERE email = 'demo.satici@tiklasat.com'),
     (SELECT id FROM categories WHERE slug = 'samsung-telefonlar'),
     'Samsung Galaxy S23 Ultra',
     '1 aylık cihaz, yurtiçi kayıtlı, sıfır ayarında.',
     'APPROVED', 85, 52000.00, now()),

    (gen_random_uuid(),
     (SELECT id FROM users WHERE email = 'demo.satici@tiklasat.com'),
     (SELECT id FROM categories WHERE slug = 'otomobil'),
     '2022 Fiat Egea Cross 1.4 Fire',
     'Hatasız boyasız, ilk sahibinden.',
     'APPROVED', 420, 950000.00, now()),

    (gen_random_uuid(),
     (SELECT id FROM users WHERE email = 'demo.satici@tiklasat.com'),
     (SELECT id FROM categories WHERE slug = 'koltuk-kanepe'),
     'IKEA Kivik 3''lü Kanepe',
     'Gri renk, kılıfları yıkanabilir. Taşınma sebebiyle satılık.',
     'APPROVED', 12, 4500.00, now()),

    (gen_random_uuid(),
     (SELECT id FROM users WHERE email = 'demo.satici@tiklasat.com'),
     (SELECT id FROM categories WHERE slug = 'oyun-konsollari'),
     'Sony PlayStation 5 Çift Kol',
     'Kozmetik olarak 10/10. CD versiyonudur.',
     'APPROVED', 560, 21000.00, now());